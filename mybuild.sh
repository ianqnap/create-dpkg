...
build_qpkg() {
	...
	log "[ ${FUNCNAME} ] (2/2) Build QPKG in QDK container"
        ...
        qbuild -v --root ${SCRIPT_PATH}/workspace/QDK/${QPKG_NAME} \
        --build-arch ${CPU_ARCH} \
        --build-version ${QPKG_VERSION}

	mv ${SCRIPT_PATH}/workspace/QDK/${QPKG_NAME}/build/*.qpkg ${SCRIPT_PATH}/release/${QPKG_FILE}.qpkg
	mv ${SCRIPT_PATH}/workspace/QDK/${QPKG_NAME}/build/*.qpkg.md5 ${SCRIPT_PATH}/release/${QPKG_FILE}.qpkg.md5
